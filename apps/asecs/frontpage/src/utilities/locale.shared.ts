...
export const getLocaleFromParams = (params?: Partial<{ country: string | null, language: string | null }>): Locale => {
  const country = params?.country || PUBLIC_DEFAULT_COUNTRY;
  const language = params?.language || baseLocale;
...
export const getLocaleFromUrl = (url: URL): Locale => {
  ...
  return getLocaleFromParams({ country, language });
}

export const getLanguage = (url: URL): string => {
  const segment = url.pathname.split('/')?.filter((s) => s).shift()?.toLowerCase();
  return segment && locales.includes(segment) ? segment : baseLocale;
}
